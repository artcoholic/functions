...
function formatTime(seconds) {
  ...
  function handleAudioPlayerState() {
    ...
    if (currentPath === '/' || currentPath === '/index.html' || currentPath === '/llm-search') {
      if (audioBtn) {
        audioBtn.style.display = 'none';
      }
      if (mAudioPlayer) {
        mAudioPlayer.style.display = 'none';
      }
    } else {
      if (audioBtn) {
 ...
 <path d="M9 18L15 12L9 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  function toggleBookmark() {
    ...
    if (existingIndex > -1) {
      ...
      bookmarks.push({ url: currentUrl, title: currentTitle, number: currentNum });
    }
  ...
  function initSearchBar() {
    ...
    // If the elements don't exist on this page, stop here.
    if (!input || !searchIcon || !clearBtn) return;

    // SWUP SAFETY CHECK: Prevent attaching multiple event listeners
    // if the search bar is in a persistent global header.
    if (input.getAttribute('data-search-initialized') === 'true') return;
