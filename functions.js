function updateYear() {
  ...
  function initializeDynamicSVGs() {
    var rawDataNodes = document.querySelectorAll('script.raw-svg-data:not(.initialized)');
		var scrollContainer = document.querySelector('.main-outer');
	
		var mobileObserverOptions = {
			root: scrollContainer, // Crucial: Binds to your custom scroll area
			// -30% top and bottom means the card only triggers when it enters the middle 40% of the screen
			rootMargin: '-30% 0px -30% 0px', 
			threshold: 0
		};

		var mobileSvgObserver = new IntersectionObserver(function (entries) {
		// Stop here if the user is on desktop (hover events will handle desktop)
			...
			entries.forEach(function (entry) {
				...
				if (entry.isIntersecting) {
					// Play when it hits the center of the viewport
					svg.unpauseAnimations();
				} else {
					// Pause and reset when it leaves the center (unless it's selected)
					if (!isSelected) {
						svg.pauseAnimations();
		...
		if (wrapperNode && wrapperNode.classList.contains('dynamic-svg-wrapper')) {
				...
				if (svg && card) {
					...
					card.addEventListener('mouseenter', function () {
						// Only fire hover play on desktop so it doesn't fight the mobile scroll observer
						if (window.innerWidth > 767) {
							svg.unpauseAnimations();
						}
					});
					...
					card.addEventListener('mouseleave', function () {
						if (window.innerWidth > 767 && !card.classList.contains('is-selected')) {
							svg.pauseAnimations();
							svg.setCurrentTime(0);
						}
					});
	
					// --- Watch for dynamic class changes (e.g. Cancel button) ---
						...
						mutations.forEach(function (mutation) {
							if (mutation.attributeName === 'class') {
								...
								if (isSelected) {
									...
									if (window.innerWidth > 767 && !isHovered) {
										svg.pauseAnimations();
										svg.setCurrentTime(0);
									} else if (window.innerWidth <= 767) {
										// Pause immediately if selection is cancelled on mobile
										svg.pauseAnimations();
										svg.setCurrentTime(0);
									}
								}
					...
					// --- Attach the Mobile Scroll Observer to this card ---
					mobileSvgObserver.observe(card);
