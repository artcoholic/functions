...
const initGlobalProgressBar = () => {
	...
	scrollContainer.addEventListener('scroll', globalScrollHandler, {
		passive: true
	});
	globalScrollHandler();
};
const initProgressBar = () => {
	...
	scrollContainer.addEventListener('scroll', scrollProgressHandler, {
		passive: true
	});
	scrollProgressHandler();
};
...
function initAudioPlayer() {
	...
	const audioSrcEl = document.getElementById('chapter-audio-src');
	if (!audioSrcEl || !audioSrcEl.value) return;
	currentAudio = new Audio(audioSrcEl.value);
	const newTitle = document.getElementById('data-chapter-title');
	const newNumber = document.getElementById('data-chapter-number');
	const mobileChTitle = document.getElementById('mobile-ch-title');
	const mobileChNum = document.getElementById('mobile-ch-number');
	if (newTitle && mobileChTitle) mobileChTitle.innerText = newTitle.value;
	if (newNumber && mobileChNum) mobileChNum.innerText = newNumber.value;
	const dPlayBtn = document.getElementById('audio-play-pause');
	const dTimeText = document.getElementById('audio-current-time');
	const dDurText = document.getElementById('audio-duration');
	const dSeekBar = document.getElementById('audio-seek-bar');
	const dSkipBack = document.getElementById('audio-skip-back');
	const dSkipFwd = document.getElementById('audio-skip-forward');
	const mPlayBtn = document.getElementById('mobile-play-pause');
	const mTimeText = document.getElementById('mobile-current-time');
	const mDurText = document.getElementById('mobile-duration');
	const mSeekBar = document.getElementById('mobile-seek-bar');
	const utilityContainer = document.querySelector('.utility-container');
	const skipAmount = 15;
	if (mSeekBar) {
		mSeekBar.value = 0;
		mSeekBar.style.setProperty('--progress', '0%');
		mSeekBar.isDragging = false;
	}
	if (mTimeText) {
		mTimeText.innerText = '0:00';
	}
	if (mPlayBtn) {
		mPlayBtn.classList.remove('is-playing');
	}
	currentAudio.addEventListener('play', () => {
		if (!currentAudio) return;
		if (dPlayBtn) dPlayBtn.classList.add('is-playing');
		if (mPlayBtn) mPlayBtn.classList.add('is-playing');
		if (utilityContainer) utilityContainer.classList.add('audio-is-playing');
	});
	currentAudio.addEventListener('pause', () => {
		if (!currentAudio) return;
		if (dPlayBtn) dPlayBtn.classList.remove('is-playing');
		if (mPlayBtn) mPlayBtn.classList.remove('is-playing');
		if (utilityContainer) utilityContainer.classList.remove('audio-is-playing');
	});
	currentAudio.addEventListener('loadedmetadata', () => {
		if (!currentAudio) return;
		const durationFormatted = formatTime(currentAudio.duration);
		if (dDurText) dDurText.innerText = durationFormatted;
		if (mDurText) mDurText.innerText = durationFormatted;
		if (dSeekBar) dSeekBar.max = currentAudio.duration;
		if (mSeekBar) mSeekBar.max = currentAudio.duration;
	});
	currentAudio.addEventListener('timeupdate', () => {
		if (!currentAudio) return;
		const timeFormatted = formatTime(currentAudio.currentTime);
		const safeDuration = isNaN(currentAudio.duration) ? 1 : currentAudio.duration;
		const percentage = (currentAudio.currentTime / safeDuration) * 100 || 0;
		if (dTimeText) dTimeText.innerText = timeFormatted;
		if (dSeekBar && !dSeekBar.isDragging) {
			dSeekBar.value = currentAudio.currentTime;
			dSeekBar.style.setProperty('--progress', `${percentage}%`);
		}
		if (mTimeText) mTimeText.innerText = timeFormatted;
		if (mSeekBar && !mSeekBar.isDragging) {
			mSeekBar.value = currentAudio.currentTime;
			mSeekBar.style.setProperty('--progress', `${percentage}%`);
		}
	});
	currentAudio.addEventListener('ended', () => {
		...
		if (dSeekBar) {
			dSeekBar.value = 0;
			dSeekBar.style.setProperty('--progress', '0%');
		}
		if (mSeekBar) {
			mSeekBar.value = 0;
			mSeekBar.style.setProperty('--progress', '0%');
	...
	if (dPlayBtn) dPlayBtn.addEventListener('click', () => (currentAudio.paused ? currentAudio.play() : currentAudio.pause()));
	// Seek Bar Logic (User dragging the slider)
	if (dSeekBar) {
		dSeekBar.addEventListener('input', () => {
			dSeekBar.isDragging = true;
			if (dTimeText) dTimeText.innerText = formatTime(dSeekBar.value);
			const max = isNaN(dSeekBar.max) || dSeekBar.max == 0 ? 1 : dSeekBar.max;
			dSeekBar.style.setProperty('--progress', `${(dSeekBar.value / max) * 100 || 0}%`);
		});
		// Bulletproof release
		['change', 'touchend', 'mouseup'].forEach((evt) => {
			dSeekBar.addEventListener(evt, () => {
				if (dSeekBar.isDragging && currentAudio) {
					try {
						// Only try to seek if the audio has actually loaded its metadata
						if (currentAudio.readyState >= 1) {
							currentAudio.currentTime = Number(dSeekBar.value);
						}
					} catch (e) {
						console.warn('Audio not ready to seek yet.');
					} finally {
						// FINALLY guarantees this runs even if the code above crashes
						dSeekBar.isDragging = false;
					}
				}
			});
		});
	...
	if (dSkipBack) {
		dSkipBack.addEventListener('click', () => {
			if (currentAudio) currentAudio.currentTime = Math.max(0, currentAudio.currentTime - skipAmount);
		});
	}
	if (dSkipFwd) {
		dSkipFwd.addEventListener('click', () => {
			if (currentAudio) {
				const dur = isNaN(currentAudio.duration) ? 0 : currentAudio.duration;
				currentAudio.currentTime = Math.min(dur, currentAudio.currentTime + skipAmount);
			}
		});
	}
}

function handleAudioPlayerState() {
	const audioBtn = document.getElementById('audio-player-btn');
	const mAudioPlayer = document.getElementById('mobile-audio-player');
	// 1. Clean up the URL by removing any trailing slashes (optional but recommended)
	const currentPath = window.location.pathname.replace(/\/$/, '') || '/';
	// 2. Store the paths where the player should be hidden in an array
	const hiddenPaths = ['/', '/index.html', '/ask-anything', '/token-types'];
	// 3. Determine the correct display state once
	const displayState = hiddenPaths.includes(currentPath) ? 'none' : '';
	// 4. Apply the state safely
	if (audioBtn) audioBtn.style.display = displayState;
	if (mAudioPlayer) mAudioPlayer.style.display = displayState;
}
let bookmarks = JSON.parse(localStorage.getItem('userBookmarks')) || [];
const globalAddBtn = document.getElementById('global-add-bookmark-btn');
const mobileAddBtn = document.getElementById('mobile-add-bookmark-btn');
const bmText1 = document.querySelectorAll('.bm-text-1');
const bmText2 = document.querySelectorAll('.bm-text-2');
const bmCounter = document.querySelectorAll('.bookmark-counter');
const badgeCounter = document.querySelector('.badge-counter');

function renderBookmarks() {
	const containers = document.querySelectorAll('.bookmark-list-container');
	if (containers.length === 0) return;
	containers.forEach((container) => {
		container.innerHTML = '';
		if (bookmarks.length === 0) {
			container.innerHTML = '<div class="empty-bookmark">No bookmarks saved yet.</div>';
			return;
		}
		[...bookmarks].reverse().forEach((bm) => {
			const item = document.createElement('div');
			item.className = 'bm-chapter-item';
			const displayChNumber = bm.number ? `<div class="text-block-16">${bm.number}</div>` : '';
			item.innerHTML = `
      	<a href="${bm.url}" class="bm-chapter-wrapper">
        	${displayChNumber}
            <div class="text-block-17">${bm.title}</div>
  			<div class="icon-16 bm-icon">
            	<svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
 <path d="M9 18L15 12L9 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
 </svg>
 			</div>
        </a>
        <button class="button remove-bookmark-btn" data-url="${bm.url}">Remove</button>
      `;
			container.appendChild(item);
		});
	});
}
// --- NEW: Check the current page state ---
function updateGlobalButtonState() {
	const currentUrl = window.location.pathname;
	const isSaved = bookmarks.find((bm) => bm.url === currentUrl);
	if (isSaved) {
		if (globalAddBtn) globalAddBtn.classList.add('is-saved');
		if (mobileAddBtn) mobileAddBtn.classList.add('is-saved');
		if (bmText1) {
			bmText1.forEach((text) => {
				text.textContent = 'unbookmark this page';
			});
		}
		if (bmText2) {
			bmText2.forEach((text) => {
				text.textContent = 'unsave current chapter';
			});
		}
	} else {
		if (globalAddBtn) globalAddBtn.classList.remove('is-saved');
		if (mobileAddBtn) mobileAddBtn.classList.remove('is-saved');
		if (bmText1) {
			bmText1.forEach((text) => {
				text.textContent = 'bookmark this page';
			});
		}
		if (bmText2) {
			bmText2.forEach((text) => {
				text.textContent = 'save current chapter';
			});
		}
	}
	if (bmCounter) {
		bmCounter.forEach((counter) => {
			counter.textContent = `${bookmarks.length}`;
		});
	}
	if (bookmarks.length === 0) {
		badgeCounter.classList.add('empty');
	} else {
		badgeCounter.classList.remove('empty');
	}
}

function toggleBookmark() {
	const currentUrl = window.location.pathname;
	const chTitleElement = document.getElementById('chapter-bm-title');
	const chNumElement = document.getElementById('chapter-bm-number');
	const currentTitle = chTitleElement ? chTitleElement.textContent : 'Home';
	const currentNum = chNumElement ? chNumElement.textContent : '';
	// Find the index of the current page in our bookmarks array
	const existingIndex = bookmarks.findIndex((bm) => bm.url === currentUrl);
	if (existingIndex > -1) {
		// If it exists (index is 0 or higher), remove it from the array
	...
	// Save, render, and update the button visually
...
function removeBookmark(url) {
	...
	localStorage.setItem('userBookmarks', JSON.stringify(bookmarks));
	renderBookmarks();
	updateGlobalButtonState(); // Update the button in case we removed the current page!
}
